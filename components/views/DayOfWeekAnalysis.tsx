...
const DayOfWeekAnalysis: React.FC<DayOfWeekAnalysisProps> = ({ store }) => {
    ...
    return (
        <div className="space-y-4">
             <div>
                <p className="text-center text-xs text-gray-400 mb-2">Analisar período:</p>
                <div className="flex justify-center bg-night-900/60 p-1 rounded-full">
                     {dateRangeTabs.map(tab => (
                        <button key={tab.id} onClick={() => setRange(tab.id)} className={`w-full py-1.5 px-2 text-xs font-semibold rounded-full transition-colors ${ range === tab.id ? 'bg-night-600 text-white shadow' : 'text-gray-300 hover:bg-night-700/50' }`}>
                            {tab.label}
                        </button>
                    ))}
                </div>
            ...
            <div>
                <p className="text-center text-xs text-gray-400 mb-2">Visualizar por:</p>
                <div className="flex justify-center bg-night-800/60 p-1 rounded-full">
                     {metricOptions.map(opt => (
                        <button key={opt.id} onClick={() => setMetric(opt.id)} className={`w-full py-1.5 px-2 text-xs font-semibold rounded-full transition-colors ${ metric === opt.id ? 'bg-brand-blue text-white shadow' : 'text-gray-300 hover:bg-night-700/50' }`}>
                            {opt.label}
                        </button>
                    ))}
                </div>
